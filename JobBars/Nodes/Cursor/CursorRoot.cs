    ...
    public unsafe class CursorRoot : NodeBase<AtkResNode> {
        ...
        private static void SetPartId( SimpleImageNode node, int partId, ref bool staticCircle ) {
            if( partId == 80 ) { // Placeholder for static circle
                if( !staticCircle ) node.LoadTexture( "ui/uld/CursorLocation.tex", JobBars.Configuration.Use4K );
                staticCircle = true;
                ...
                if( staticCircle ) node.LoadTexture( "ui/uld/IconA_Recast2.tex", JobBars.Configuration.Use4K );
                staticCircle = false;
